...
func (p *Paginator) Page() int {
  ...
  s:=strings.Split(path,"/")
   re:=regexp.MustCompile("[0-9]+")
	...
	u:=re.FindString(s[2])
	if(strings.Index(s[2],"_")>-1){
	 u="0"  
	}
	if u==""{
	 u="0"
	}
	p.page, _ = strconv.Atoi(u)
...
func (p *Paginator) PageLink(page int) string {
	...
	if strings.Index(s[2],"_") >-1{
	  ...
	  link=  "/"+s[1]+"/"+strconv.Itoa(page)+"/"+s[2]
	}else{
	 ...
	 if u==""{
	  if(page!=1){
	     s[2]=s[2]+strconv.Itoa(page)
	  }
	
	 } else{
	 rpl:=""
	 if(page!=1){
	  rpl=strconv.Itoa(page)
	 }
	 s[2]=strings.Replace(s[2],u,rpl,1)
	 }
	 link= "/"+s[1]+"/"+s[2]+"/"+s[3];
	}
